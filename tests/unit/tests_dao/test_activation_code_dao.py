...
class TestActivationCodeDAO:
    ...
    async def test_setup_indexes_creates_all_indexes(self, activation_code_collection, subtests):
    ...
    async def test_setup_indexes_prevents_duplicate_codes(self, activation_code_collection, code_factory_data):
        with allure.step("Инициализация индексов"):
            await ActivationCodeDAO.setup_indexes(activation_code_collection)
    ...
    @tag(FeaturesActivationCodeMark.CREATE_CODE)
    async def test_create_code_returns_id(self, activation_code_dao: ActivationCodeDAO, code_factory_data, subtests):
    ...
    async def test_get_code_returns_code(
        self, activation_code_dao: ActivationCodeDAO, created_activation_code: dict, subtests
    ):
    ...
    @tag(FeaturesActivationCodeMark.GET_CODE)
    async def test_get_code_returns_none_if_not_found(self, activation_code_dao: ActivationCodeDAO):
    ...
    async def test_delete_code_returns_true(
        self, activation_code_dao: ActivationCodeDAO, created_activation_code: dict
    ):
        code_id = str(created_activation_code["_id"])
    ...
    @tag(FeaturesActivationCodeMark.DELETE_CODE)
    async def test_delete_code_returns_false_if_not_found(self, activation_code_dao: ActivationCodeDAO):
        fake_code_id = event_faker.generate_random_id()
    ...
    async def test_use_code_marks_code_as_used(
        self, activation_code_dao: ActivationCodeDAO, created_activation_code: dict, created_user: dict, subtests
    ):
    ...
    async def test_use_code_returns_none_for_nonexistent_code(
        self, activation_code_dao: ActivationCodeDAO, created_user: dict
    ):
    ...
    async def test_use_code_returns_none_for_already_used_code(
        self, activation_code_dao: ActivationCodeDAO, activation_code_factory, created_user: dict
    ):
    ...
    async def test_get_codes_returns_list(
        self, activation_code_dao: ActivationCodeDAO, activation_codes_factory, subtests
    ):
        ...
        with allure.step("Получение списка кодов"):
            codes = await activation_code_dao.get_codes(skip=DEFAULT_PAGINATION_SKIP, limit=DEFAULT_PAGINATION_LIMIT)
    ...
    async def test_get_codes_with_pagination(
        ...
        with allure.step("Получение первой страницы"):
            page1 = await activation_code_dao.get_codes(skip=0, limit=page_size)

        with allure.step("Получение второй страницы"):
            page2 = await activation_code_dao.get_codes(skip=page_size, limit=page_size)
    ...
    async def test_get_codes_sorted_by_created_at_desc(
        self, activation_code_dao: ActivationCodeDAO, activation_codes_factory
    ):
        with allure.step("Создание кодов с разными временами создания"):
            base_time = datetime.now(timezone.utc)
            times = [base_time - timedelta(days=2), base_time - timedelta(days=1), base_time]
            codes = await activation_codes_factory(count=len(times), created_at_list=times)
    ...
    async def test_get_codes_filter_by_is_used(
        ...
        with allure.step("Создание кодов с разными is_used"):
            await activation_codes_factory(count=DEFAULT_CODES_COUNT, is_used_list=[True, False, True])
    ...
    async def test_get_codes_filter_by_role(
        self, activation_code_dao: ActivationCodeDAO, activation_code_factory, subtests
    ):
    ...
    async def test_get_codes_filter_by_created_at_from(
        ...
        with allure.step("Создание кодов с разными датами создания"):
            ...
            await activation_codes_factory(count=DEFAULT_CODES_COUNT, created_at_list=[past, middle, future])
