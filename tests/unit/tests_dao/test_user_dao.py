...
class TestUserDAO:
    ...
    async def test_create_user_returns_id(self, mock_user_dao: UserDAO, subtests):
    ...
    async def test_get_user_by_id_returns_user(self, mock_user_dao: UserDAO, created_user: dict, subtests):
    ...
    async def test_update_user_updates_fields(self, mock_user_dao: UserDAO, created_user: dict, subtests):
    ...
    async def test_get_users_returns_list(self, base_users: UserDAO, subtests):
    ...
    async def test_get_users_with_filter_role(self, base_users: UserDAO, created_user: dict):
        with allure.step("Смена роли пользователю"):
            await base_users.update_user_role(user_id=created_user["_id"], new_role=UserRoleEnum.ORGANIZER)
    ...
    @tag(FeaturesUserMark.GET_USERS)
    async def test_get_users_specific_quantity(self, mock_user_dao: UserDAO, subtests):
    ...
    async def test_get_users_with_filter_created_at_from(
        self, mock_user_dao: UserDAO, users_with_different_dates: dict
    ):
        dao = mock_user_dao
    ...
    @tag(FeaturesUserMark.GET_USERS)
    async def test_get_users_with_filter_created_at_to(self, mock_user_dao: UserDAO, users_with_different_dates: dict):
        dao = mock_user_dao
        now = users_with_different_dates["now"]
    ...
    @tag(FeaturesUserMark.GET_USERS)
    async def test_get_users_with_filter_date_range(self, mock_user_dao: UserDAO, users_with_different_dates: dict):
        dao = mock_user_dao
        now = users_with_different_dates["now"]
    ...
    async def test_create_user_with_duplicate_email_raises_error_after_setup_indexes(
        self, user_collections: AsyncMongoMockCollection
    ):
        await UserDAO.setup_indexes(user_collections)
