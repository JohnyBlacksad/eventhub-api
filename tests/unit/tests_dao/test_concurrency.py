...
class TestActivationCodeDAOConcurrency:
    ...
    async def test_concurrent_create_unique_codes(
        self, activation_code_dao: ActivationCodeDAO, setup_indexes_for_activation_code, subtests
    ):
        created_codes = []

        async def create_unique_code():
            code_data = {"role": UserRoleEnum.ORGANIZER, "code": str(uuid4())}
    ...
    async def test_concurrent_read_write_code(
        ...
        with allure.step("Одновременное чтение и использование кода"):
            tasks = [read_code() for _ in range(CONCURRENT_OPERATIONS_COUNT)] + [use_code()]
            await asyncio.gather(*tasks, return_exceptions=True)
