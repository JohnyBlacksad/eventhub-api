...
class TestRegistrationDAO:
    ...
    async def test_setup_indexes_creates_all_indexes(self, registration_collection, subtests):
    ...
    async def test_setup_indexes_prevents_duplicate_registrations(
        self, registration_collection, created_user: dict, created_event: dict
    ):
        with allure.step("Инициализация индексов"):
            await RegistrationDAO.setup_indexes(registration_collection)
    ...
    async def test_add_registration_returns_insert_result(
        self, registration_dao: RegistrationDAO, created_user: dict, created_event: dict, subtests
    ):
        user_id = str(created_user["_id"])
        event_id = str(created_event["_id"])
    ...
    async def test_remove_registration_returns_delete_result(
        self, registration_dao: RegistrationDAO, created_registration: dict, subtests
    ):
    ...
    async def test_remove_registration_returns_zero_if_not_found(
        self, registration_dao: RegistrationDAO, created_user: dict, created_event: dict
    ):
        user_id = str(created_user["_id"])
        event_id = str(created_event["_id"])
    ...
    async def test_get_event_registrations_returns_list(
        self, registration_dao: RegistrationDAO, created_event: dict, event_registrations_factory, subtests
    ):
        event_id = str(created_event["_id"])
    ...
    async def test_get_event_registrations_sorted_by_registered_at(
        self, registration_dao: RegistrationDAO, created_event: dict, user_factory
    ):
    ...
    @tag(FeaturesRegistrationMark.GET_EVENT_REGISTRATIONS)
    async def test_get_event_registrations_returns_empty_for_nonexistent_event(self, registration_dao: RegistrationDAO):
    ...
    async def test_get_user_registrations_returns_list(
        self, registration_dao: RegistrationDAO, created_user: dict, event_factory_for_user, subtests
    ):
        user_id = str(created_user["_id"])
    ...
    async def test_get_user_registrations_with_limit(
        self, registration_dao: RegistrationDAO, created_user: dict, event_factory_for_user
    ):
        user_id = str(created_user["_id"])
    ...
    @tag(FeaturesRegistrationMark.GET_USER_REGISTRATIONS)
    async def test_get_user_registrations_returns_empty_for_nonexistent_user(self, registration_dao: RegistrationDAO):
    ...
    @tag(FeaturesRegistrationMark.DELETE_ALL_REGISTRATIONS_FOR_EVENT)
    async def test_delete_all_registrations_for_event_returns_false(self, registration_dao: RegistrationDAO):
        fake_event_id = event_faker.generate_random_id()
    ...
    async def test_set_deletion_time_for_event_updates_registrations(
        ...
        with allure.step("Проверка что deleted_at установлен"):
            ...
            for reg in registrations:
                ...
                actual_ts = (
                    reg["deleted_at"].timestamp()
                    if reg["deleted_at"].tzinfo
                    else reg["deleted_at"].replace(tzinfo=timezone.utc).timestamp()
                )
    ...
    async def test_set_deletion_time_for_event_with_none(
        self, registration_dao: RegistrationDAO, created_event: dict, event_registrations_factory
    ):
        event_id = str(created_event["_id"])
        death_date = datetime.now(timezone.utc) + timedelta(hours=1)

        with allure.step("Создание регистраций и установка deleted_at"):
    ...
    @tag(FeaturesRegistrationMark.DELETE_REGISTRATION_BY_USER)
    async def test_delete_registration_by_user_returns_false(self, registration_dao: RegistrationDAO):
        fake_user_id = faker.generate_user_id()
