...
from app.schemas.token import RefreshTokenModel, TokenModel
from app.schemas.users import UserLoginModel, UserRegisterModel
...
async def register(
    ...
    return TokenModel(accessToken=access_token, refreshToken=refresh_token)
...
async def login(
    ...
    """
    user = await user_service.authenticate_user(email=login_data.email, password=login_data.password.get_secret_value())
...
async def refresh_token(refresh_token: RefreshTokenModel, auth_service: AuthService = Depends(get_auth_service)):
    ...
    return TokenModel(accessToken=new_access, refreshToken=new_refresh)
