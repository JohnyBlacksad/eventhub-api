...
async def create_event(
    ...
    """
    response = await event_service.create_event(event_data=event_data, user_id=str(current_user.id))
...
async def update_event(
    ...
    response = await event_service.update_event(
        event_id=event_id, user_id=str(current_user.id), update_data=update_data
    )
...
async def delete_registration(
    ...
    """
    response = await event_service.unregister_from_event(event_id=event_id, user_id=str(current_user.id))


@event_router.get("/{event_id}/participants", response_model=list[dict], status_code=status.HTTP_200_OK)
async def get_event_participants(
    event_id: str, skip=Query(0, ge=0), limit=Query(10, le=50), event_service: EventService = Depends(get_event_service)
):
    ...
    """
    response = await event_service.get_event_participants(event_id=event_id, skip=skip, limit=limit)
...
async def get_my_registrations(
    ...
    """
    response = await event_service.get_user_registrations(user_id=str(current_user.id))
